...
const dashboardCardClassName = "border border-default-200 bg-surface";
...
function addMetric(target: TrafficMinuteMetric, source: TrafficMinuteMetric) {
  ...
  target.averageDurationMs =
    target.count > 0 ? Math.round(target.totalDurationMs / target.count) : 0;
}
...
function aggregateHourlyTimeline(points: TrafficTimelinePoint[]) {
  ...
  return Array.from(buckets.values()).map((point) => {
    point.page.averageDurationMs =
      point.page.count > 0
        ? Math.round(point.page.totalDurationMs / point.page.count)
        : 0;
    point.api.averageDurationMs =
      point.api.count > 0
        ? Math.round(point.api.totalDurationMs / point.api.count)
        : 0;
    point.thirdParty.averageDurationMs =
      point.thirdParty.count > 0
        ? Math.round(point.thirdParty.totalDurationMs / point.thirdParty.count)
        : 0;
  ...
  return days.reduce<TrafficTotals>(
    (accumulator, day) => {
      ...
      accumulator.page.averageDurationMs =
        accumulator.page.count > 0
          ? Math.round(
              accumulator.page.totalDurationMs / accumulator.page.count,
            )
          : 0;
      ...
      accumulator.api.averageDurationMs =
        accumulator.api.count > 0
          ? Math.round(accumulator.api.totalDurationMs / accumulator.api.count)
          : 0;
      ...
      accumulator.thirdParty.averageDurationMs =
        accumulator.thirdParty.count > 0
          ? Math.round(
              accumulator.thirdParty.totalDurationMs /
                accumulator.thirdParty.count,
            )
          : 0;

      accumulator.all.count +=
        day.page.count + day.api.count + day.thirdParty.count;
      accumulator.all.totalDurationMs +=
        day.page.totalDurationMs +
        day.api.totalDurationMs +
        day.thirdParty.totalDurationMs;
      accumulator.all.successCount +=
        day.page.successCount +
        day.api.successCount +
        day.thirdParty.successCount;
      accumulator.all.failCount +=
        day.page.failCount + day.api.failCount + day.thirdParty.failCount;
      accumulator.all.averageDurationMs =
        accumulator.all.count > 0
          ? Math.round(accumulator.all.totalDurationMs / accumulator.all.count)
          : 0;
      return accumulator;
    },
    {
...
function MetricCard({
  ...
  return (
    <Card
      className="border border-default-200 bg-surface shadow-sm"
      styles={{ body: { padding: 20 } }}
      title={
        <div className="grid grid-cols-[2.8rem_minmax(0,1fr)] items-start gap-3">
          <span
            className={`flex h-11 w-11 items-center justify-center rounded-2xl ${accent} bg-current/10`}
          >
...
function MinuteSummary({
  ...
  const successRate =
    metric.count > 0
      ? (metric.successCount /
          Math.max(1, metric.successCount + metric.failCount)) *
        100
      : 0;

  return (
    <div className="rounded-2xl border border-(--ant-color-border) bg-background/70 px-4 py-4">
      ...
      <p className="mt-2 text-2xl font-semibold tracking-tight text-foreground">
        {formatCount(metric.count)} 次
      </p>
      <p className="mt-1 text-xs leading-5 text-default-500">
        成功率 {formatPercent(successRate)} · 平均{" "}
        {metric.averageDurationMs > 0
          ? `${Math.round(metric.averageDurationMs)}ms`
          : "0ms"}
      </p>
...
function TimelineChart({ points }: { points: TrafficTimelinePoint[] }) {
  const peak = Math.max(
    1,
    ...points.map(
      (point) => point.page.count + point.api.count + point.thirdParty.count,
    ),
  );
        ...
        <div className="flex items-end gap-2">
          {points.map((point, index) => {
            ...
            const barHeight = Math.max(
              4,
              Math.round((totalCount / peak) * 144),
            );
            const pageHeight =
              totalCount > 0
                ? Math.max(2, Math.round((pageCount / totalCount) * barHeight))
                : 0;
            const apiHeight =
              totalCount > 0
                ? Math.max(2, Math.round((apiCount / totalCount) * barHeight))
                : 0;
            const thirdPartyHeight = Math.max(
              0,
              barHeight - pageHeight - apiHeight,
            );
            const showLabel =
              points.length <= 12 ||
              index % 2 === 0 ||
            ...
            return (
              <div
                key={`${point.dayKey}-${point.minuteKey}`}
                className="flex w-[38px] flex-col items-center gap-2"
              >
                <div className="flex h-36 w-full flex-col justify-end overflow-hidden rounded-full bg-default-100/80">
                  ...
                  {apiHeight > 0 ? (
                    <div
                      className="w-full bg-cyan-500/85"
                      style={{ height: `${apiHeight}px` }}
                    />
                  ) : null}
                  {pageHeight > 0 ? (
                    <div
                ...
                <p className="h-8 text-[10px] leading-4 text-default-400">
...
export function StatsDashboard({ overview }: StatsDashboardProps) {
  ...
  const apiShare =
    totals.all.count > 0
      ? (totals.all.successCount /
          Math.max(1, totals.all.successCount + totals.all.failCount)) *
        100
      : 0;
    ...
    <div className="relative overflow-hidden bg-background">
      <section className="relative mx-auto w-full max-w-6xl px-4 py-8 md:px-6 lg:px-8">
        <header className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_360px]">
          <Card
            className={`${dashboardCardClassName} shadow-lg`}
          ...
          >
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                  ...
                  <p className="max-w-3xl text-sm leading-7 text-default-600 md:text-base">
                    统计页面访问、API 流量和第三方资源站请求，保留最近 7
                    天，按分钟聚合。页面与接口耗时会一起呈现，方便快速判断流量高峰和失败波动。
                  </p>
                </div>
            ...
            <div className="grid gap-5 p-6 pt-5 md:p-8 md:pt-5">
              <div className="grid gap-3 md:grid-cols-3">
                <MinuteSummary
                  label="当前页面分钟"
                  metric={currentMinute.page}
                  tone="bg-accent"
                />
                <MinuteSummary
                  label="当前 API 分钟"
                  metric={currentMinute.api}
                  tone="bg-cyan-500"
                />
                <MinuteSummary
                  label="当前第三方分钟"
                  metric={currentMinute.thirdParty}
                  tone="bg-amber-500"
                />
              </div>
            ...
            <Card className={dashboardCardClassName}>
              ...
              <div className="grid gap-3 px-5 pb-5 text-center">
                <div className="relative min-h-[8rem] w-full overflow-hidden rounded-2xl bg-accent/10 px-6 py-4">
                  <p className="absolute left-6 top-4 text-xs font-medium uppercase tracking-[0.24em] text-default-500">
                    总请求
                  </p>
                ...
                <p className="text-sm text-default-600">
                  平均{" "}
                  {totals.all.averageDurationMs > 0
                    ? `${Math.round(totals.all.averageDurationMs)}ms`
                    : "0ms"}{" "}
                  · 成功率 {formatPercent(apiShare)}
                </p>
        ...
        <Card className={`mt-8 ${dashboardCardClassName}`}>
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-[0.24em] text-default-400">
                  <th className="border-b border-(--ant-color-border) px-3 py-3 font-medium">
                    日期
                  </th>
                  <th className="border-b border-(--ant-color-border) px-3 py-3 font-medium">
                    页面
              ...
              <tbody>
                {dailySummaries.map((day) => {
                  ...
                  const dayFail =
                    day.page.failCount +
                    day.api.failCount +
                    day.thirdParty.failCount;
                  const successRate =
                    dayTotal > 0
                      ? (daySuccess / Math.max(1, daySuccess + dayFail)) * 100
                      : 0;
                  ...
                  return (
                    ...
                    >
                      <td className="border-b border-(--ant-color-border) px-3 py-3 font-medium text-foreground">
                        {formatDayLabel(day.dayKey)}
                      </td>
                      <td className="border-b border-(--ant-color-border) px-3 py-3 text-default-600">
                        {formatCount(day.page.count)} 次
                      </td>
                      <td className="border-b border-(--ant-color-border) px-3 py-3 text-default-600">
                        {formatCount(day.api.count)} 次
                      </td>
                      <td className="border-b border-(--ant-color-border) px-3 py-3 text-default-600">
                        {formatCount(day.thirdParty.count)} 次
                      </td>
                      <td className="border-b border-(--ant-color-border) px-3 py-3 text-default-600">
                        {formatPercent(successRate)}
                      </td>
                      <td className="border-b border-(--ant-color-border) px-3 py-3 text-default-600">
                        {averageDuration > 0 ? `${averageDuration}ms` : "0ms"}
                      </td>
                    </tr>
                  );
                })}
